...
Status: accepted design; amended by PD-132 on 12 September 2026 and PD-134 on 13 September 2026;
...
library may implement its approved design. Spike-gated WM-043 later feeds the same events into an
...
starts only after its separate feasibility gate, cannot replace the text/2.5D fallback and must pass
its capability exit before commercial release.

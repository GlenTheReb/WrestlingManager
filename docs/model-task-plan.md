...
TEW-scale company operations, structured creative tools, a living industry, guaranteed text-led
show presentation, a conditional retro 3D broadcast expansion and native custom content. The rows are capability briefs, not automatically
...
The accepted 168-item destination baseline is
[`docs/game-product-rules.md`](game-product-rules.md). Every future execution ticket cites the relevant
rule IDs; former PD-107–131 questions are resolved at product level, while ticket-specific constants,
catalogues and balance evidence remain to be defined and tested.
...
- Match and angle presentation remains simulation-driven. Smart arena lighting, effects, scene cards,
  crowd state and commentary lead through a complete text/2.5D viewer. PD-132 adds an original
  PlayCanvas Engine v2 retro 3D broadcast through `@playcanvas/react` after a successful WebGL2/60 FPS
  spike. It never owns simulation or saving; PD-134 makes a viable implementation a commercial-release
  gate without adding theme-song/titantron scope.
- PD-112 makes live direction part of authoritative incremental simulation: workers are consulted,
  referee access comes from executed sequences, and accepted changes rebuild only the unsimulated
  future. Quick Sim uses contingencies; 2.5D/3D highlights and realtime share the same decisions.
...
This table is not a 43-branch serial queue. “Depends on” describes what the complete capability must
...
[WM-020 execution record](tickets/WM-020.1.md). WM-022, WM-023 and the WM-021.1 Talent Search
foundation are merged, with WM-021.1 landing through PR #5 at `28f6a26`. WM-021.2 is the drafted
company Roster execution ticket. See the
[canonical search rules](search-and-discovery-rules.md). WM-025 is merged through PR #6 at `61d2ec1`.
WM-024.1 is implemented, review-fixed and verified locally; an explicit commit remains.
Later-domain search registration remains
...
|               6 | WM-006 | Worker-aware road-agent planning, referee relay and live adaptation                                 | WM-005, PD-112                                        | Sol/High              | 2–4 wk                |
|               7 | WM-007 | Move chains, communication windows, future replanning and explicit finishes                         | WM-006                                                | Sol/High              | 3–5 wk                |
...
|              14 | WM-021 | Talent Search/identity integration merged; WM-021.2 company Roster remains                          | WM-020, WM-022–023, WM-025                            | Incremental           | Size on authorisation |
|              15 | WM-025 | Person/Character separation, ring identities, gimmicks, masks and alignment — merged through PR #6  | WM-022–023, PD-106                                    | Sol/Medium            | Complete              |
|              16 | WM-024 | Current-data Person profile, career and action hub — implemented, review-fixed and verified locally | WM-020–023, WM-025, PD-133                            | Sol/Medium            | Commit pending        |
...
|              22 | WM-030 | Player persona/career, ownership, goals, hierarchy, booking team and figurehead                     | WM-011; WM-022–025 for full persona integration       | Sol/High              | Size by slice         |
...
|              30 | WM-012 | Fictional-world generation plus relationship, market and historical-event foundation                | Early bootstrap; WM-011, WM-031–034, WM-042 for exit  | Sol/High              | Size by slice         |
...
|              33 | WM-039 | Early Global Search shell; later condensed/full CPU-show viewing                                    | WM-017, WM-021 early; WM-013, WM-034, WM-036–038 full | Sol/High              | 3–5 wk                |
...
|              37 | WM-043 | Release-gated PlayCanvas v2 retro 3D broadcast and main move-family engine                          | WM-013–014; later WM-007, WM-029, WM-034, WM-041      | Sol/High              | 26–39 wk core         |
|              38 | WM-015 | Save recovery, migration/compatibility policy and long-save hardening                               | Continuous; final gate after WM-040                   | Sol/High              | 3–5 wk                |
|              39 | WM-016 | Cohesive WM visual identity and complex-screen design system                                        | Early on proven screens; WM-014, WM-035 for exit      | Sol/High              | 3–5 wk                |
|              40 | WM-017 | Game front door, settings, career setup, navigation and accessible workflows                        | Early shell; WM-012, WM-016, WM-030, WM-041 for exit  | Terra/High            | 3–5 wk                |
|              41 | WM-018 | Regression, property, balance, performance, accessibility and soak suite                            | Continuous; final gate after WM-015–017               | Terra/High            | 4–7 wk                |
|              42 | WM-041 | Early database selection/import safety; later native editor/export and optional asset packs         | WM-012/017 early; public tools after WM-015–018       | Sol/High              | 4–7 wk                |
|              43 | WM-019 | Installer, licensing, documentation and release-candidate cleanup                                   | WM-041                                                | Terra/Medium          | 2–3 wk                |

## Capability and execution-ticket rules

WM-002–043 are destination-level capability briefs. Most are intentionally larger than one safe
...
[`docs/product-decisions.md`](product-decisions.md) and
[`docs/game-product-rules.md`](game-product-rules.md), not guessed inside code. Existing detailed
records for WM-001 and WM-020–023 remain authoritative where they are more specific.

## Schedule estimate

These are capability-area sizing ranges, not a calendar forecast and not values to add into a project
finish date. Capabilities overlap, land through vertical slices and include uncertainty at very
different levels. Asset production, playtesting, review turnaround and scope discovered during
implementation can extend them.
...
| Presentation contract and guaranteed viewer | WM-013–014                     |                  5–8 weeks |
| Release-gated retro 3D broadcast core       | WM-043                         |                26–39 weeks |
| Hardening, interface, modding and release   | WM-015–019, WM-041             |                19–32 weeks |

The social-interstitial amendment is contained within the existing WM-036, WM-013 and WM-014
estimates; it adds no separate schedule bucket. The ranges exclude final content authoring, licensed
assets, community testing, marketing and review turnaround. Each capability area must contribute to
playable milestones; [the milestone plan](milestones.md), not these horizontal sizing buckets,
governs delivery and integration.

A calendar estimate is produced only after at least three representative execution tickets have
measured end-to-end throughput including review and fixes. Forecast the next playable milestone from
that evidence, then revise it as scope and velocity change. Keep WM-043 separate: its 26–39 week range
describes the main move-family/show-engine work only after its bounded feasibility spike succeeds;
broad release-quality 3D polish remains a separate 52–78 week risk range. Do not add these ranges to
the other rows or present the result as a promised project finish date.
